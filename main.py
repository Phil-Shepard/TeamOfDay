...
#todo функция для переноса из одного статуса в другой, работает с ошибкой
    def on_combobox_select(event):
        true_status = 0
        if book in LibraryWillRead: true_status = 'Запланировано'
        if book in LibraryNowRead: true_status = 'Читаю'
        if book in LibraryWasRead: true_status = 'Прочитано'
        if book in LibraryWillRead and status.current() != 0:
            ...
            true_status = ['Запланировано',"Читаю","Прочитано"][status.current()]
    ...
    #todo выпадающий список статуса, работает некооректно
